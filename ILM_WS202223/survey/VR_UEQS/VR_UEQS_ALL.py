...
if __name__ == '__main__':
    ...
    dataPoint = pd.read_csv("VR_UEQS_Point.csv", delimiter=';')
    xPoint, yPoint = runAnalyze(dataPoint)

    dataGaze = pd.read_csv("VR_UEQS_Gaze.csv", delimiter=';')
    xGaze, yGaze = runAnalyze(dataGaze)

    dataGrab = pd.read_csv("VR_UEQS_Grab.csv", delimiter=';')
    xGrab, yGrab = runAnalyze(dataGrab)

    dataSn = pd.read_csv("VR_UEQS_Sn.csv", delimiter=';')
    xSn, ySn = runAnalyze(dataSn)

    dataSf = pd.read_csv("VR_UEQS_Sf.csv", delimiter=';')
    xSf, ySf = runAnalyze(dataSf)

    dataTeleport = pd.read_csv("VR_UEQS_Teleport.csv", delimiter=';')
    xTele, yTele = runAnalyze(dataTeleport)
